...
restart:
    SDIO->ICR = (uint32_t) 0xFFFFFFFF;
    sdio_transfer_failed = 0;//WICED_FALSE;
    ++attempts;
    ...
    {
    	//ANN temp
        result = 1051; //WWD_SDIO_RETRIES_EXCEEDED;
        goto exit;
    }
...
//ANN: deferred
    ...
    if ((response_r1 & SDMMC_R5_ERRORBITS) == SDMMC_ALLZERO)
    {
        if (data != NULL)
        {
            *data = response_r1; //response_r1 of 4162 could not get assigned here...check..
        }
        return SDMMC_ERROR_NONE;
    }
...
//ANN commented below for sometime
